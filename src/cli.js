...
function getBareRef(ref: NodeRef): string {
  return ref.ref === 'Optional' ? getBareRef(ref.of) : ref.ref === 'List' ? getBareRef(ref.of) : ref.name;
}

function getBareRefTarget(ref: NodeRef): 'Node' | 'NodeGroup' {
  return ref.ref === 'Optional' || ref.ref === 'List' ? getBareRefTarget(ref.of) : ref.ref;
}
...
function validate(grammar: Grammar, options: ProgramOptions) {
    ...
    for (const ref of nodeGroup.members) {
      ...
      invariant(
        get(grammar.nodesByName, memberName) ||
          (nodeGroup.name !== memberName && !!get(grammar.nodeGroupsByName, memberName)),
  ...
  for (const node of grammar.nodes) {
    for (const field of node.fields) {
      ...
      referenced.add(bare);
      invariant(
        options.isBuiltIn(bare) || !!grammar.nodeGroupsByName[bare] || !!get(grammar.nodesByName, bare),
  ...
  invariant(defined.size === 0, `The following node kinds are never referenced: ${Array.from(defined).join(', ')}`);
}
...
function generateTypeCheckCondition(
  ...
  if (expected.ref === 'Optional') {
    conditions.push(
      [`${actualValue} === null`, generateTypeCheckCondition(grammar, expected.of, actualValue, options)]
        .filter(Boolean)
        .map((s) => `(${s})`)
        .join(' || '),
    );
    ...
    conditions.push(
      `${actualValue}.every(item => ${generateTypeCheckCondition(grammar, expected.of, 'item', options)})`,
    );
    ...
    conditions.push(
      `${actualValue}.${options.discriminator} === ${options.transformDiscriminator(JSON.stringify(expected.name))}`,
    );
  }
...
function parseGrammarDefinition(inputFile: string): Grammar {
  ...
  for (const nodeKey of Object.keys(nodesByName)) {
    ...
    node.fields = Object.keys(node.fieldsByName).map((fieldKey) => node.fieldsByName[fieldKey]);
  }
...
function generateCode(grammar: Grammar, options: ProgramOptions): string {
  ...
  for (const nodeGroup of grammar.nodeGroups) {
    const [subNodes, subGroups] = partition(nodeGroup.members, (ref) => getBareRefTarget(ref) === 'Node');
    const conditions = subNodes
      .map((ref) => `node.${discriminator} === ${transformDiscriminator(JSON.stringify(getBareRef(ref)))}`)
  ...
  output.push(`
        ...
        function isNode(node: Node): boolean %checks {
            return (
                ${grammar.nodes
                  .map((node) => `node.${discriminator} === ${transformDiscriminator(JSON.stringify(node.name))}`)
                  .join(' || ')}
            )
        }
    `);
  ...
  for (const node of grammar.nodes) {
    output.push(`
            export type ${node.name} = {|
                ...
                ${node.fields.map((field) => `${field.name}: ${getTypeScriptType(field.ref)}`).join(', ')}
            |}
        `);
  }
  ...
  for (const node of grammar.nodes) {
    const optionals = new Set(
      takeWhile(
        ...
        (field) => field.ref.ref === 'Optional' || (field.ref.ref === 'List' && field.ref.min === 0),
    ...
    const argChecks = node.fields
      .map((field) => {
        return `invariant(${generateTypeCheckCondition(
          ...
          field.name,
          options,
        )}, \`Invalid value for "${field.name}" arg in "${node.name}" call.\\nExpected: ${serializeRef(
          field.ref,
...
function writeFile(contents: string, path: string) {
  const existing = fs.existsSync(path) ? fs.readFileSync(path, { encoding: 'utf-8' }) : null;
...
function run() {
  ...
  if (program.args.length < 1) {
    ...
    const outputFile = program.args[1] || program.args[0].replace(/.grammar$/, '') + '.js';
    ...
    function transformDiscriminator(value: string): string {
      if (opts.discriminatorTransform === undefined) {
        ...
        throw new Error('Unknown --discriminator-transform value. Should be upper or lower');
      }
    }
